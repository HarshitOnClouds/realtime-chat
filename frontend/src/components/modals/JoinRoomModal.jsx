    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white bg-opacity-10 backdrop-blur-lg p-6 rounded-2xl w-96 border border-white border-opacity-20">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-white text-sm mb-2">Room Code</label>
            <input
              type="text"
              placeholder="Enter 8-character room code"
              value={roomCode}
              onChange={(e) => setRoomCode(e.target.value.toUpperCase())}
              className="w-full px-4 py-3 bg-white bg-opacity-20 border border-white border-opacity-30 rounded-lg text-white placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-50 uppercase tracking-wider text-center text-lg font-mono"
              required
              maxLength={8}
              autoFocus
            />
          ...
          <div className="flex space-x-2">
            <button
              ...
              className="flex-1 py-3 bg-linear-to-r from-purple-500 to-pink-500 text-white rounded-lg font-semibold hover:from-purple-600 hover:to-pink-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
        ...
        <div className="mt-4 p-3 bg-white bg-opacity-5 rounded-lg">
          <p className="text-white text-opacity-60 text-sm">
            💡 Get the room code from someone who has already created or joined the room.
          </p>
        </div>
      </div>
    </div>
